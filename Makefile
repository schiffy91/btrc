...
BTRCC_GENERATED_AST := src/compiler/python/ast_nodes.py src/compiler/btrc/ast/node.btrc
...
ast-generate: ## Regenerate the Python AST node classes from ast.asdl
	@set -euo pipefail; \
		target=src/compiler/python/ast_nodes.py; \
...
ast-generate-btrc: ## Regenerate the btrc AST node classes from ast.asdl
	@set -euo pipefail; \
		target=src/compiler/btrc/ast/node.btrc; \
		tmp=$$(mktemp "$${target}.tmp.XXXXXX"); \
		trap 'rm -f "$$tmp"' EXIT; \
