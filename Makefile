.PHONY: all help build package wheel btrcc btrcc-release-c btrcc-macos-arm64 btrcc-macos-x64 btrcc-linux-x64 btrcc-linux-arm64 \
        btrcc-windows-x64 btrcc-dist test-windows gpu gpu-required gui stubs-generate ast-generate ast-generate-btrc \
        test test-unit test-lsp test-debug test-btrc test-btrc-selfhost test-selfhost bootstrap test-c11 test-generate-goldens \
        generated-check hosted-abi-generate hosted-abi-check lint format format-check \
...
build: hosted-abi-check ## Create bin/btrcpy wrapper script
	@mkdir -p bin
	@printf '%s\n' \
		'#!/usr/bin/env python3' \
		'import sys, os' \
...
BTRCC_BUNDLER := $(NIX) python3 -m src.compiler.python.btrcc_bundle
...
gpu-required: gpu ## Build GPU runtime library; fail when production dependencies are missing
	@$(NIX) bash -c 'archive=src/stdlib/gpu/build/libbtrc_gpu.a; \
		test -f "$$archive" || { \
...
gui: ## Build GUI runtime (software renderer always; window backend needs GLFW)
	@$(NIX) bash -c '\
		...
		archive="$$D/build/libbtrc_gui.a" && object="$$D/build/btrc_gui.o" && \
		rm -f "$$archive" "$$object" && \
	...
	@$(NIX) bash -c '\
		D=src/stdlib/gui && mkdir -p "$$D/build" && \
		archive="$$D/build/libbtrc_gui_window.a" && object="$$D/build/btrc_gui_window.o" && \
		rm -f "$$archive" "$$object" && \
		trap "rm -f \"$$archive\" \"$$object\"" EXIT && \
	...
	@$(NIX) bash -c '\
		D=src/stdlib/gui && mkdir -p "$$D/build" && \
		archive="$$D/build/libbtrc_gui_font.a" && object="$$D/build/btrc_gui_font.o" && \
		rm -f "$$archive" "$$object" && \
		trap "rm -f \"$$archive\" \"$$object\"" EXIT && \
...
test: hosted-abi-check gpu-required ## Run everything: unit + LSP + debugger + language corpus on BOTH compilers
	$(NIX) $(PYTEST) src/tests/ src/devex/lsp/tests/ src/devex/debug/tests/ \
		--ignore=src/tests/btrc/test_bootstrap.py $(PYTEST_ARGS)
	$(NIX) $(PYTEST) src/tests/btrc/test_bootstrap.py $(PYTEST_SERIAL_ARGS)

test-unit: hosted-abi-check ## Run Python reference-compiler unit tests (lexer, parser, analyzer, codegen)
	$(NIX) $(PYTEST) src/tests/python/ $(PYTEST_ARGS)

test-lsp: hosted-abi-check ## Run the editor/LSP server tests (reuses the compiler)
	$(NIX) $(PYTEST) src/devex/lsp/tests/ $(PYTEST_ARGS)

test-debug: hosted-abi-check ## Run the debugger (DAP adapter) tests (needs lldb + a C compiler)
	$(NIX) $(PYTEST) src/devex/debug/tests/ $(PYTEST_ARGS)

test-selfhost: hosted-abi-check ## Verify the self-hosted lexer is byte-identical to btrcpy
	$(NIX) bash src/compiler/btrc/verify_lex.sh

test-btrc: hosted-abi-check ## Language corpus through the Python reference compiler (fast)
	$(NIX) $(PYTEST) src/tests/runner.py --compilers=python $(PYTEST_ARGS)

test-btrc-selfhost: hosted-abi-check ## Language corpus through the self-hosted compiler (btrcc) + btrc-specific tests
	$(NIX) $(PYTEST) src/tests/runner.py --compilers=btrc src/tests/btrc/ \
		--ignore=src/tests/btrc/test_bootstrap.py $(PYTEST_ARGS)

bootstrap: hosted-abi-check ## Prove the self-hosted compiler reproduces itself bit-for-bit (fixed point)
	$(NIX) $(PYTEST) src/tests/btrc/test_bootstrap.py -v $(PYTEST_SERIAL_ARGS)

test-c11: hosted-abi-check gpu-required btrcc ## Strict C11: both compilers with gcc + clang at -O0 through -O3
	@$(NIX) bash -c '\
...
test-generate-goldens: hosted-abi-check ## Regenerate golden .stdout files
	$(NIX) python3 src/tests/generate_expected.py

hosted-abi-generate: ## Regenerate self-hosted hosted-ABI policy tables
	$(NIX) python3 -m src.compiler.python.gen_hosted_abi_btrc

hosted-abi-check: ## Check self-hosted hosted-ABI policy tables
	$(NIX) python3 -m src.compiler.python.gen_hosted_abi_btrc --check

generated-check: ## Check every committed generated source without modifying it
	$(NIX) python3 -m src.compiler.python.check_generated

lint: hosted-abi-check ## Run generated-policy checks and ruff linter
	$(NIX) ruff check src/
...
examples: hosted-abi-check gpu-required ## Build and run all examples
	$(NIX) $(MAKE) -C examples all

examples-todo: hosted-abi-check ## Build the todo example
	$(NIX) $(MAKE) -C examples todo

examples-game: hosted-abi-check gpu-required ## Build the 3D engine game
	$(NIX) $(MAKE) -C examples game

examples-triangle: hosted-abi-check gpu-required ## Build the GPU triangle example
	$(NIX) $(MAKE) -C examples triangle

examples-sgd: hosted-abi-check gpu-required ## Build the GPU SGD example
	$(NIX) $(MAKE) -C examples sgd

examples-gui: hosted-abi-check ## Build + run the headless GUI example
	$(NIX) $(MAKE) -C examples gui

bench: hosted-abi-check ## Build + run the benchmark suite (times transpile + compile + run)
	$(NIX) $(MAKE) -C bench
...
clean: ## Remove all build artifacts
	...
	rm -rf build/generated/ build/out/ build/lib/ build/bdist.*/ build/btrcc/ build/temp.*/
