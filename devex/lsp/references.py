...
from devex.lsp.definition import (
    _find_token_at_position,
    _find_token_index,
    _resolve_object_class,
    DefinitionMap,
)
...
def _classify_symbol(
    ...
    token_idx = _find_token_index(tokens, token)
...
def _find_member_references(
    ...
    for tok in matching:
        tok_idx = _find_token_index(tokens, tok)
...
def get_references(
    ...
    if kind == 'class':
        ...
        locs = _find_member_references(
            class_name, member_name, kind,
            result.tokens, result, class_table, dmap, include_declaration,
        )
    else:
        locs = _find_variable_references(name, result.tokens)

    return [
        _btrc_to_lsp_location(result.uri, line, col, len(name))
...
def prepare_rename(
    ...
    keywords = {
        ...
        'char', 'true', 'false', 'null', 'new', 'delete', 'self',
        'break', 'continue', 'switch', 'case', 'default', 'try', 'catch',
        'throw', 'do', 'List', 'Map', 'Set',
    }
    if token.value in keywords:
        return None

    start = lsp.Position(line=max(0, token.line - 1), character=max(0, token.col - 1))
    end = lsp.Position(line=max(0, token.line - 1), character=max(0, token.col - 1 + len(token.value)))
