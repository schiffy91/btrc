...
- **Platform code lives in a platform subfolder of its group**
  (`Audio/MacOS`, `GUI/MacOS`, `GUI/FreeType`, `Image/MacOS`, `Tray/Linux`,
...
| --- | --- |
| `App`, `UI`, `GUI`, `Tray` | Application lifecycle, the BTRC-drawn declarative toolkit, native windows/controls/fonts, system tray |
| `Audio`, `Realtime` | Device enumeration, duplex sessions, realtime clocks, clip transport; `Audio/MacOS` is the CoreAudio provider |
| `GPU` | WebGPU device, programs, uniform buffers, image textures, surface rendering, readback, compute dispatch |
| `Image` | DDS decoding, platform image decoding through system image services, pixel access |
