...
typedef struct GPUWindow GPUWindow;
...
void* btrc_gpu_create_render_pipeline(void* gpu, void* shader, char* vert_entry, char* frag_entry);
...
bool GPUWindow_isOpen(GPUWindow* self);
void GPUWindow_poll(GPUWindow* self);
int GPUWindow_width(GPUWindow* self);
int GPUWindow_height(GPUWindow* self);
GPUShader* GPU_createShader(GPU* self, char* wgsl);
GPURenderPipeline* GPU_createPipeline(GPU* self, GPUShader* shader);
bool GPU_beginFrame(GPU* self, float r, float g, float b);
void GPU_draw(GPU* self, GPURenderPipeline* pipeline, int vertexCount);
void GPU_endFrame(GPU* self);
...
void GPUWindow_init(GPUWindow* self, char* title, int width, int height) {
    self->__rc = 1;
    (self->_handle = btrc_gpu_window_create(title, width, height));
}

GPUWindow* GPUWindow_new(char* title, int width, int height) {
    GPUWindow* self = ((GPUWindow*)malloc(sizeof(GPUWindow)));
    memset(self, 0, sizeof(GPUWindow));
    GPUWindow_init(self, title, width, height);
    return self;
}

void GPUWindow_destroy(GPUWindow* self) {
...
void GPUShader_init(GPUShader* self, void* h) {
    self->__rc = 1;
    (self->_handle = h);
}

GPUShader* GPUShader_new(void* h) {
    GPUShader* self = ((GPUShader*)malloc(sizeof(GPUShader)));
    memset(self, 0, sizeof(GPUShader));
    GPUShader_init(self, h);
    return self;
}

void GPUShader_destroy(GPUShader* self) {
    if (self->_handle != NULL) {
...
void GPURenderPipeline_init(GPURenderPipeline* self, void* h) {
    self->__rc = 1;
    (self->_handle = h);
}

GPURenderPipeline* GPURenderPipeline_new(void* h) {
    GPURenderPipeline* self = ((GPURenderPipeline*)malloc(sizeof(GPURenderPipeline)));
    memset(self, 0, sizeof(GPURenderPipeline));
    GPURenderPipeline_init(self, h);
    return self;
}

void GPURenderPipeline_destroy(GPURenderPipeline* self) {
    if (self->_handle != NULL) {
...
void GPU_init(GPU* self, GPUWindow* window) {
    self->__rc = 1;
    (self->_handle = btrc_gpu_init(window->_handle));
}

GPU* GPU_new(GPUWindow* window) {
    GPU* self = ((GPU*)malloc(sizeof(GPU)));
    memset(self, 0, sizeof(GPU));
    GPU_init(self, window);
    return self;
}

void GPU_destroy(GPU* self) {
    if (self->_handle != NULL) {
...
GPUShader* GPU_createShader(GPU* self, char* wgsl) {
    return GPUShader_new(btrc_gpu_create_shader(self->_handle, wgsl));
}

GPURenderPipeline* GPU_createPipeline(GPU* self, GPUShader* shader) {
    return GPURenderPipeline_new(btrc_gpu_create_render_pipeline(self->_handle, shader->_handle, "vs_main", "fs_main"));
}
...
int main(void) {
    GPUWindow* win = GPUWindow_new("btrc Triangle", 800, 600);
    GPU* gpu = GPU_new(win);
    GPUShader* shader = GPU_createShader(gpu, "\n        @vertex fn vs_main(@builtin(vertex_index) i: u32) -> @builtin(position) vec4f {\n            var pos = array<vec2f, 3>(\n                vec2f( 0.0,  0.5),\n                vec2f(-0.5, -0.5),\n                vec2f( 0.5, -0.5)\n            );\n            return vec4f(pos[i], 0.0, 1.0);\n        }\n\n        @fragment fn fs_main() -> @location(0) vec4f {\n            return vec4f(1.0, 0.0, 0.0, 1.0);\n        }\n    ");
    GPURenderPipeline* pipeline = GPU_createPipeline(gpu, shader);
    while (GPUWindow_isOpen(win)) {
        GPUWindow_poll(win);
        if (GPU_beginFrame(gpu, 0.0, 0.0, 0.0)) {
            GPU_draw(gpu, pipeline, 3);
            GPU_endFrame(gpu);
        }
    }
    if (gpu != NULL) {
