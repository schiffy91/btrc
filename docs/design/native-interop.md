...
The full design is not yet implemented. Both compilers have experimental C-call consumers; managed ownership, Objective-C/C++ adapters and provider migration remain unfinished. BTRC should call real native APIs through typed imports, with provider policy and lifetime management written in BTRC. Native object identity and layout are preserved; wrappers do not mirror objects into unrelated pointer bags.
...
Both compilers now have an experimental C-call import path. The pinned LibTooling reader supplies SDK scalar types, typedefs, opaque identities, constants and parameter names; normal visibility, type checking and structured IR consume them. SDK headers provide the C declarations—no handwritten signature wrappers. Explicit declaration provenance keeps SDK reserved names separate from user naming rules.

The reader also preserves record layouts, callbacks, per-level qualifiers and ownership in the shared native model. That metadata is not yet a callable or managed source type: unsupported projections and annotated ownership fail explicitly. Unannotated pointers remain raw. Native imports bypass artifact caching until transitive headers/toolchain enter its key.

Build the reader with `nix build .#btrc-native-header`. See [configuration and current limits](../../src/language/package-manifest.md#experimental-native-header-imports). Enabled verification:

```sh
BTRC_NATIVE_HEADER_READER=<built-package>/bin/btrc-native-header \
python3 -m pytest src/tests/python/test_native_import_consumer.py src/tests/python/test_native_header_reader.py
```

Both consumers execute actual CoreFoundation create/query/release under strict C11 and Apple ASan/UBSan. The final enabled run passes 147 SDK-consumer, reference/self-host package and compiler-structure checks using a fresh self-hosted compiler. It covers wrong calls, import isolation, SDK names/typedefs, unsupported semantics, target matching, define values and no partial C on failure. Generated-source, focused lint and formatting checks pass. Prior decoder parity passed 62 checks; neither result qualifies managed lifetimes or the full compiler matrix.

Next: native type/lifetime semantics and their real SDK execution tests, then the first migrated provider. Checkpoint 1 remains incomplete; no managed ownership, Objective-C/C++ adapters, broad stdlib migration or full compiler qualification is claimed.
