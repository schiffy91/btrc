...
The full design is not yet implemented. The reference compiler has an experimental C-call consumer; self-hosted calls, managed ownership and provider migration remain unfinished. BTRC should call real C, Objective-C and C++ APIs through typed imports, with provider policy and lifetime management written in BTRC. Native object identity and layout are preserved; wrappers do not mirror objects into unrelated pointer bags.
...
Current executable checkpoint: the reference compiler invokes the reader with an explicit SDK/target and imports supported declarations through ordinary visibility, analysis and structured IR. A BTRC CoreFoundation create/query/release program runs under strict C11 and Apple ASan/UBSan without copied signatures. Wrong calls, import leakage, SDK parameter names and stale transitive headers are regression-tested. Raw unannotated pointers are **not managed owners**; explicit ownership annotations and unrepresentable native types are rejected. This does not complete checkpoint 1. Artifact caching is disabled for native imports until headers/toolchain enter its key. Self-hosted calls remain unavailable. Next: port the call path and implement native type/lifetime semantics, then migrate the first provider—not more metadata-only verification. Configuration and limits are in [the manifest documentation](../../src/language/package-manifest.md#experimental-native-header-imports).
...
Both package resolvers model module-owned [`native.bindings`](../../src/language/package-manifest.md#experimental-native-header-imports): header, language/standard, exact selected names and target predicates. They reject escaping paths, malformed requests and overlapping exports. Loaded-module/target projection selects requests; ordinary headers and linker-plan schema 1 are unchanged. Self-hosted compilation and an unconfigured reference compiler still reject active bindings explicitly rather than silently discarding them.

The reader JSON remains experimental. `src/language/native_abi.asdl` generates a separate native semantic model for both frontends; the ordinary source AST is unchanged. `NativeHeaderCodec` / `FeNativeHeaderCodec` check and decode it without erasing pointer-level qualifiers, declaration identity or explicit ownership. Layout/count values remain exact decimal strings, never JSON floating-point values. Self-hosted call integration, native ownership enforcement, Objective-C/C++ adapters and provider migration remain to implement. BTRC's current flattened `TypeExpr` cannot represent every native pointer qualifier/layout; reject unsupported shapes rather than projecting away those semantics. Record metadata describes the SDK ABI, not permission to manually clone its objects or manufacture managed ownership.
