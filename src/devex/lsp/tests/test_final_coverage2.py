"""Final reachable-branch coverage: static/inherited/any-class signature
...
def test_signature_any_class_fallback():
    # n is an int; act() lives on Foo — resolution falls back to searching all classes
    src = ("class Foo { public int f; public Foo() { self.f = 0; }\n"
           "            public int act(int x) { return x; } }\n"
           "int main() { int n = 5; return n.act(2); }\n")
    s = get_signature_help(analyze(src), pos_of(src, "n.act(2", offset=6))
    assert s is not None and "act" in s.signatures[0].label
