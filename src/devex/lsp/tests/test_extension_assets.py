...
def test_extension_packaging_stages_lsp_payload(tmp_path):
    ...
    isolated_import = subprocess.run(
        [
            ...
            "-c",
            "import runpy; "
            f"runpy.run_path({str(bundle_root / 'src/devex/lsp/server.py')!r}, run_name='packaged_probe')",
        ],
        cwd=tmp_path,
        capture_output=True,
        text=True,
...
def test_extension_packaging_replaces_read_only_previous_staging(tmp_path):
    script_path = EXT_DIR / "scripts" / "prepare_lsp_package.py"
    spec = importlib.util.spec_from_file_location("prepare_lsp_package_replacement", script_path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    ...
    ext_dir = tmp_path / "ext"
    ext_dir.mkdir()
    first = module.prepare(ext_dir=ext_dir, repo_root=REPO_ROOT)
    ...
    second = module.prepare(ext_dir=ext_dir, repo_root=REPO_ROOT)
...
def test_extension_packaging_rejects_distribution_root_metadata(tmp_path, monkeypatch, metadata_root):
    script_path = EXT_DIR / "scripts" / "prepare_lsp_package.py"
    spec = importlib.util.spec_from_file_location("prepare_lsp_package_metadata", script_path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
    ...
    with pytest.raises(RuntimeError, match="unsafe path"):
        module._vendor_runtime_dependencies(tmp_path / "vendor")
...
def test_extension_packaging_removes_staging_symlink_without_touching_target(tmp_path):
    script_path = EXT_DIR / "scripts" / "prepare_lsp_package.py"
    spec = importlib.util.spec_from_file_location("prepare_lsp_package_symlink", script_path)
    module = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(module)
