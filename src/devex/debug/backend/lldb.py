"""LLDB session ownership for the btrc debug adapter.
...
them across, applying btrc-aware value rendering from :mod:`.values`.
"""
...
from .values import BtrcValuePresenter


class LldbSession:
    def __init__(self, lldb_module, *, value_presenter=None):
        self.lldb = lldb_module
        self.value_presenter = BtrcValuePresenter() if value_presenter is None else value_presenter
    ...
    def terminate(self):
        if self.process and self.process.IsValid():
            if self.process.GetState() in {
                ...
                return
            self._require_success(self.process.Kill(), "terminate process")
    ...
    def set_breakpoints(self, source_path, specs):
        ...
        source_location, source_key = self._source_identity(source_path)
    ...
    def _interpolate(self, frame, msg):
        def repl(m):
            ...
            if val and val.IsValid() and not val.GetError().Fail():
                return self.value_presenter.summarize(val)
            return m.group(0)
    ...
    def threads(self):
        ...
        for t in self.process:
            out.append({"id": t.GetThreadID(), "name": self._thread_name(t)})
        return out
    ...
    def variables(self, var_ref):
        ...
        return [self._var_dict(child, name) for name, child in self.value_presenter.children(payload)]

    def _var_dict(self, sbval, name_override=None):
        name = name_override if name_override is not None else (sbval.GetName() or "?")
        value_str = self.value_presenter.summarize(sbval)
        ref = 0
        if self.value_presenter.children(sbval):
            ref = self._alloc()
    ...
    def evaluate(self, frame_id, expression):
        ...
        if not val.IsValid() or val.GetError().Fail():
            ...
            translated = self._dot_to_arrow(expression)
            if translated != expression:
                val = frame.EvaluateExpression(translated)
            if not val.IsValid() or val.GetError().Fail():
                return None
        ref = 0
        if self.value_presenter.children(val):
            ref = self._alloc()
            self._vars[ref] = ("value", val)
        return {
            "result": self.value_presenter.summarize(val),
            "type": val.GetTypeName() or "",
            "variablesReference": ref,
        }
    ...
    def cont(self):
        self._require_success(self.process.Continue(), "continue process")
    ...
    def pause(self):
        self._require_success(self.process.Stop(), "pause process")

    @staticmethod
    def _dot_to_arrow(expression):
        """Translate BTRC member access without changing float literals."""
        return re.sub(
    ...
    @staticmethod
    def _require_success(error, action):
