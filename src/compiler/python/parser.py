...
from .tokens import Token, TokenType, TYPE_KEYWORDS
...
class Parser:
    ...
    def _parse_property(self, access, type_expr, name, line, col) -> PropertyDecl:
        """Parse a C#-style property: type name { get; set; } or { get { ... } set { ... } }"""
        self._expect(TokenType.LBRACE)
        has_getter = False
        has_setter = False
        getter_body = None
        ...
        while not self._check(TokenType.RBRACE) and not self._at_end():
            ...
            if tok.type == TokenType.IDENT and tok.value == "get":
                ...
                if self._match(TokenType.SEMICOLON):
                    getter_body = None  # auto-property getter
                ...
                has_setter = True
                if self._match(TokenType.SEMICOLON):
                    setter_body = None  # auto-property setter
                elif self._check(TokenType.LBRACE):
    ...
    def _is_type_start(self, tok: Token) -> bool:
        ...
        if tok.type == TokenType.IDENT:
            return True
        if tok.type == TokenType.LIST or tok.type == TokenType.MAP or tok.type == TokenType.SET or tok.type == TokenType.ARRAY:
            return True
        # Tuple type: (int, int)
    ...
    def _is_tuple_type_start(self) -> bool:
        ...
        if tok.type not in TYPE_KEYWORDS and tok.type not in (TokenType.LIST, TokenType.MAP, TokenType.SET, TokenType.ARRAY):
            self.pos = save
            return False
    ...
    def _is_var_decl_start(self) -> bool:
        ...
        # Built-in btrc types
        if tok.type in (TokenType.LIST, TokenType.MAP, TokenType.SET, TokenType.ARRAY):
            return self._lookahead_is_var_decl()
        ...
        try:
            ...
            if tok.type == TokenType.LPAREN:
                ...
                if self.tokens[self.pos].type == TokenType.IDENT:
                 ...
                 tok.type in (TokenType.LIST, TokenType.MAP, TokenType.SET, TokenType.ARRAY):
                self.pos += 1
            else:
                self.pos = save
                return False
    ...
    def _parse_for_stmt(self):
        ...
        if (self._check(TokenType.IDENT) and self._peek(1).type == TokenType.COMMA
                and self._peek(2).type == TokenType.IDENT and self._peek(3).type == TokenType.IN):
            var_name = self._advance().value
            ...
            iterable = self._parse_expr()
            body = self._parse_block()
            return ForInStmt(var_name=var_name, var_name2=var_name2, iterable=iterable,
                             body=body, line=tok.line, col=tok.col)
    ...
    def _is_cast(self) -> bool:
        ...
        if tok.type in TYPE_KEYWORDS or tok.type in (TokenType.LIST, TokenType.MAP, TokenType.SET, TokenType.ARRAY):
            # Find the matching )
            depth = 1
            self.pos += 1
            while self.pos < len(self.tokens) and depth > 0:
    ...
    def _is_verbose_lambda(self) -> bool:
        ...
        Lookahead to find 'function' keyword after a type expression."""
        save = self.pos
        try:
            ...
            # Skip base type
            tok = self.tokens[self.pos]
            if tok.type in TYPE_KEYWORDS or tok.type == TokenType.IDENT or \
               tok.type in (TokenType.LIST, TokenType.MAP, TokenType.SET, TokenType.ARRAY):
                self.pos += 1
            else:
                self.pos = save
    ...
    def _is_arrow_lambda(self) -> bool:
        ...
        Must see typed params and '=>' after closing ')'."""
        save = self.pos
        try:
            self.pos += 1  # skip (
            # Empty params: () =>
            if self.tokens[self.pos].type == TokenType.RPAREN:
                ...
                self.pos = save
                return result
            # Check first param: must be type name pattern
            tok = self.tokens[self.pos]
            if tok.type not in TYPE_KEYWORDS and tok.type != TokenType.IDENT and \
               tok.type not in (TokenType.LIST, TokenType.MAP, TokenType.SET, TokenType.ARRAY):
                self.pos = save
                return False
            # Scan for the matching ), then check for =>
            depth = 1
            self.pos += 1
            while self.pos < len(self.tokens) and depth > 0:
                t = self.tokens[self.pos]
    ...
    def _parse_arrow_lambda(self) -> LambdaExpr:
        ...
        if self._check(TokenType.LBRACE):
            ...
            expr = self._parse_expr()
            body = Block(statements=[ReturnStmt(value=expr, line=expr.line, col=expr.col)],
                         line=expr.line, col=expr.col)
