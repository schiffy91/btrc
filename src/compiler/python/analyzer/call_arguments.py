...
class CallArgumentBindingMixin:
    ...
    def _validate_call_arity(self, name, params, args, names, line, col):
        ...
        if len(args) < required:
            self.context.error(f"'{name}()' expects at least {required} argument(s) but got {len(args)}", line, col)
        elif len(args) > len(params):
            self.context.error(f"'{name}()' expects at most {len(params)} argument(s) but got {len(args)}", line, col)
    ...
    def _validate_named_call(self, name, params, args, names, line, col):
        ...
        for argument_name in names:
            if argument_name:
                ...
                if argument_name not in parameter_names:
                    self.context.error(f"'{name}()' has no parameter named '{argument_name}'", line, col)
                    continue
                parameter_index = parameter_names.index(argument_name)
                if parameter_index in supplied:
                    self.context.error(f"'{name}()' got argument '{argument_name}' more than once", line, col)
                supplied.add(parameter_index)
                continue
            if saw_named:
                self.context.error(f"'{name}()' positional argument follows named argument", line, col)
                continue
        ...
        for index, param in enumerate(params):
            if index not in supplied and getattr(param, "default", None) is None:
                self.context.error(f"'{name}()' missing required argument '{param.name}'", line, col)
