...
class CEmitter:
    ...
    def emit(self, module: IRModule) -> str:
        ...
        self._line_filenames = {}
        return self._emit_unit(module, -1, 0, len(module.function_defs))

    def emit_units(self, module: IRModule, target_lines: int, units_prefix: str | None = None) -> list[str]:
        ...
        return [
            self._emit_unit(
                module, unit, first, starts[unit + 1] if unit + 1 < len(starts) else len(sizes), units_prefix
            )
    ...
    def _emit_unit(
        self, module: IRModule, unit_index: int, first: int, end: int, units_prefix: str | None = None
    ) -> str:
    ...
    def _emit_line_directive(self):
        ...
        self._lines.append(f'#line {line} "{self._line_filenames[filename]}"')
