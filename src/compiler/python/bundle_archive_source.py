...
def _entry_identity(entry: ArchiveEntry) -> FileIdentity:
...
def _validate_regular_identity(entry: ArchiveEntry, descriptor: int) -> None:
    opened = os.fstat(descriptor)
    current = entry.path.lstat()
    expected = _entry_identity(entry)
    if (
...
def _hash_stream(stream: BinaryIO, max_size: int | None = None) -> ContentSnapshot:
    digest = hashlib.sha256()
    size = 0
    while chunk := stream.read(_CHUNK_SIZE if max_size is None else min(_CHUNK_SIZE, max_size - size + 1)):
        digest.update(chunk)
        size += len(chunk)
...
def _discover_regular(
    ...
    try:
        _validate_regular_identity(provisional, descriptor)
        with os.fdopen(descriptor, "rb") as stream:
            descriptor = -1
            content = _hash_stream(stream, expected_size)
            _validate_regular_identity(provisional, stream.fileno())
            return content
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def _entry(path: Path, metadata: os.stat_result, *, expected_size: int | None = None) -> ArchiveEntry:
    ...
    if expected_size is not None and metadata.st_size != expected_size:
        raise ValueError(f"archive source changed size before validation: {path}")
    return ArchiveEntry(
        path=path,
        device=metadata.st_dev,
        inode=metadata.st_ino,
...
def bounded_bundle_entries(
    ...
    for path in paths:
        ...
        entries.append(_entry(path, path.lstat(), expected_size=files.get(relative)))
    return entries
...
@contextmanager
def open_regular(entry: ArchiveEntry) -> Iterator[_HashingReader]:
    if entry.content is None:
        raise ValueError(f"archive source is not a regular file: {entry.path}")
    descriptor = os.open(entry.path, _open_flags())
    try:
        _validate_regular_identity(entry, descriptor)
        with os.fdopen(descriptor, "rb") as stream:
            descriptor = -1
            before = _hash_stream(stream, entry.size)
            ...
            yield emitted
            stream.seek(0)
            after = _hash_stream(stream, entry.size)
            _validate_regular_identity(entry, stream.fileno())
...
def validate_directory(entry: ArchiveEntry) -> None:
...
def validate_payload(entry: ArchiveEntry, payload: bytes) -> None:
...
def validate_bundle_snapshot(
    ...
    *,
    ...
    if files is None or directories is None:
        ...
        current = bounded_bundle_entries(bundle, files, directories)
    if len(current) != len(expected):
        raise ValueError(f"archive source tree changed while packaging: {bundle}")
