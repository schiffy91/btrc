"""Bounded structural readers for native ``btrcc`` executables."""
...
def _bounded_address_extent(offset: int, size: int, limit: int = _UINT64_MAX) -> bool:
    """Return whether adding ``size`` to ``offset`` stays in the address type."""

    return 0 <= offset <= limit and 0 <= size <= limit - offset


def _contains(offset: int, size: int, point: int) -> bool:
    """Return whether ``point`` lies in a non-empty extent without addition."""

    return size > 0 and offset <= point and point - offset < size


def _elf64_machine(reader: _Reader) -> int:
    ...
    for _ in range(program_count):
        fields = struct.unpack("<IIQQQQQQ", reader.read_exact(program_size))
        segment_kind, flags, file_offset, virtual_address, _, file_size, memory_size, _ = fields
        file_extents.append((file_offset, file_size))
        if segment_kind == 1:
            if file_size > memory_size or not _bounded_address_extent(virtual_address, memory_size):
                raise InvalidBinary
            if flags & 1 and _contains(virtual_address, file_size, entry):
                executable_entry = True
    total = reader.finish()
    if not executable_entry or not all(_bounded_extent(*extent, total) for extent in file_extents):
        raise InvalidBinary
    if section_count and not _bounded_extent(section_offset, section_size * section_count, total):
        raise InvalidBinary
    return machine


def _mach_o64_machine(reader: _Reader) -> int:
    header = reader.read_exact(_MACH_HEADER_SIZE)
    magic, machine, _, kind, command_count, command_bytes, _, reserved = struct.unpack("<IIIIIIII", header)
    if (
        magic != 0xFEEDFACF
        or kind != 2
        or reserved != 0
    ...
    for _ in range(command_count):
        ...
        if command == 0x19:
            if size < 72:
                raise InvalidBinary
            virtual_address, virtual_size, file_offset, file_size = struct.unpack_from("<QQQQ", commands, cursor + 24)
            _, initial_protection, section_count = struct.unpack_from("<III", commands, cursor + 56)
            if (
                size != 72 + section_count * 80
                or file_size > virtual_size
                or not _bounded_address_extent(virtual_address, virtual_size)
            ):
                raise InvalidBinary
            file_extents.append((file_offset, file_size))
            ...
            if size != 24:
                raise InvalidBinary
            main_entry = struct.unpack_from("<Q", commands, cursor + 8)[0]
    ...
    if (
        ...
        or not main_entry
        or not any(_contains(offset, size, main_entry) for offset, size in executable_extents)
        or not all(_bounded_extent(*extent, total) for extent in file_extents)
    ):
        raise InvalidBinary
    return machine


def _pe32_plus_machine(reader: _Reader) -> int:
    ...
    machine, section_count, _, symbol_offset, symbol_count, optional_size, characteristics = struct.unpack_from(
    ...
    for _ in range(section_count):
        ...
        if (
            not _bounded_address_extent(virtual_address, mapped_size, _UINT32_MAX)
            or virtual_address > image_size
            or mapped_size > image_size - virtual_address
        ):
            raise InvalidBinary
        if section_flags & 0x20000000 and _contains(virtual_address, raw_size, entrypoint):
            executable_entry = True
    total = reader.finish()
    if (
        not executable_entry
        or header_size > total
        or not all(size == 0 or _bounded_extent(offset, size, total) for offset, size in file_extents)
        or (symbol_count and not _bounded_extent(symbol_offset, symbol_count * 18, total))
    ):
        raise InvalidBinary
    return machine


_READERS = {"elf": _elf64_machine, "mach-o": _mach_o64_machine, "pe": _pe32_plus_machine}


def read_executable_machine(stream: BinaryIO, binary_format: str) -> int | None:
    """Return the machine id only for one complete supported executable."""

    reader = _Reader(stream)
    try:
        machine = _READERS[binary_format](reader)
    except (InvalidBinary, KeyError):
        machine = None
    try:
        reader.finish()
