...
def cleanup_scope_entry(gen: IRLowerer, marker: str | None) -> list[IRStmt]:
    if marker is None:
        return []
...
def cleanup_scope_exit(gen: IRLowerer, marker: str | None) -> list[IRStmt]:
    if marker is None:
        return []
...
def control_cleanup_exit(gen: IRLowerer, targets: set[str]) -> list[IRStmt]:
