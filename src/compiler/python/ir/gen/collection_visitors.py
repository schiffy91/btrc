...
def emit_generic_visitor(
    ...
    body = [
        ...
        *_builtin_visit_body(gen, base, arguments, type_renderer),
    ]
...
def _builtin_visit_body(
    ...
    return _list_visit_body(gen, arguments[0], type_renderer)


def _indexed_loop(
    gen,
    bound: str,
    slots: list[tuple[str, TypeExpr]],
    type_renderer: CTypeRenderer,
) -> IRFor:
    ...
    for field_name, slot_type in slots:
        ...
        body.extend(
            slot_visit_stmts(
                gen,
                slot_type,
                IRIndex(obj=array, index=IRVar(name=index)),
                type_renderer,
            )
        )
    return IRFor(
...
def _occupied_loop(
    ...
    loop = _indexed_loop(gen, "cap", slots, type_renderer)
...
def _list_visit_body(
    ...
    for field_name in ("head", "tail"):
        ...
        body.extend(slot_visit_stmts(gen, node_type, field, type_renderer))
    return body
