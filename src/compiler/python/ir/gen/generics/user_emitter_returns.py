...
def lower_generic_return(emitter, statement):
    ...
    value = emitter._expr(statement.value) if statement.value else None
    ...
    if isinstance(statement.value, Identifier):
        ...
        if managed_return and local_owned:
            returned_local = statement.value.name
    expression_owned = bool(managed_return and (local_owned or emitter._owns_expr(statement.value)))
