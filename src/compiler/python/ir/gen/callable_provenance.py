...
def callable_return_abi(context, expression) -> str:
    """Return the proven managed-return ABI for one callable expression."""
    if isinstance(expression, LambdaExpr):
        return OWNED_RETURN
    if isinstance(expression, Identifier):
        lexical = context.callable_return_abis.get(expression.name)
        if lexical is not None:
            return lexical
        declaration = context.analyzed.function_table.get(expression.name)
    ...
    if isinstance(expression, FieldAccessExpr):
        declaration = _source_static_method(context, expression)
        if declaration is not None and declaration.body is not None:
            return OWNED_RETURN
    if isinstance(expression, CastExpr):
        return callable_return_abi(context, expression.expr)
    if isinstance(expression, UnaryExpr) and expression.op == "&":
        return callable_return_abi(context, expression.operand)
    if isinstance(expression, TernaryExpr):
        return join_return_abis(
            callable_return_abi(context, expression.true_expr),
            callable_return_abi(context, expression.false_expr),
        )
    return BORROWED_RETURN


def _source_static_method(context, expression: FieldAccessExpr):
    ...
    if not isinstance(receiver, Identifier):
        return None
    class_info = context.analyzed.class_table.get(receiver.name)
...
def callable_has_owned_return_abi(gen, expression) -> bool:
    ...
    return callable_return_abi(gen.context, expression) == OWNED_RETURN
...
def bind_local_callable(gen, name: str, type_expr, initializer) -> None:
    ...
    if resolved is None or resolved.base != "__fn_ptr":
        gen.context.callable_return_abis.pop(name, None)
        gen.context.callable_types.pop(name, None)
        return
    gen.context.callable_types[name] = resolved
    gen.context.callable_return_abis[name] = (
        callable_return_abi(gen.context, initializer) if initializer is not None else BORROWED_RETURN
    )
...
def bind_callable_abi(gen, name: str, type_expr, return_abi: str) -> None:
    ...
    if resolved is not None and resolved.base == "__fn_ptr":
        gen.context.callable_return_abis[name] = return_abi
...
def rebind_local_callable(gen, assignment) -> None:
    ...
    if assignment.op != "=" or not isinstance(assignment.target, Identifier):
        return
    if assignment.target.name not in gen.context.callable_return_abis:
        return
    ...
    if resolved is None or resolved.base != "__fn_ptr":
        return
    gen.context.callable_return_abis[assignment.target.name] = callable_return_abi(
        gen.context,
        assignment.value,
    )
...
def begin_exceptional_callable_capture(gen):
    ...
    capture = (frozenset(gen.context.callable_return_abis), [])
...
def _record_exceptional_callable_flow(gen) -> None:
    current = gen.context.callable_return_abis
...
def begin_callable_scope(gen):
    ...
    enclosing = (
        gen.context.callable_return_abis.copy(),
        gen.context.callable_types.copy(),
    )
...
def finish_callable_scope(gen, enclosing) -> None:
    ...
    gen.context.callable_return_abis = result
    gen.context.callable_types = enclosing_types


def snapshot_callable_flow(gen) -> dict[str, str]:
    return gen.context.callable_return_abis.copy()


def restore_callable_flow(gen, state: dict[str, str]) -> None:
    gen.context.callable_return_abis = state.copy()
...
def join_callable_flows(gen, *states: dict[str, str]) -> None:
    ...
    gen.context.callable_return_abis = joined
