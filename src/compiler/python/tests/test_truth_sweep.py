...
Covers three audit findings:
  ...
  CMP-24: a raw pointer-to-builtin (string*) and a generic collection
          (List<string>) are incompatible, while unknown<->unknown C-interop
          pairs stay permissive.
"""
...
def compile_and_run(source: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(c_path, "w") as f:
            f.write(c_code)
        build = subprocess.run(["gcc", "-std=c11", "-pedantic-errors",
                                c_path, "-o", exe, "-lm"],
...
def test_for_in_range_unaffected_when_no_user_range():
    out = compile_and_run(
        "int main() { for i in range(3) { print(i); } return 0; }\n")
...
def test_every_spec_helper_exists_in_runtime_registry():
    ...
    # Conversions name only C stdlib functions, never __btrc helpers
    for name, (c_func, _cast) in STRING_CONVERSIONS.items():
        assert name in STRING_METHODS
        assert not c_func.startswith("__btrc")
...
# ---------------------------------------------------------------------------
# CMP-24: pointer-to-builtin vs generic collection rejected; C interop kept
# ---------------------------------------------------------------------------
...
def test_string_pointer_not_compatible_with_generic_collection():
    list_str = TypeExpr(base="List", generic_args=[TypeExpr(base="string")],
                        pointer_depth=1)
...
def test_var_inferred_split_keeps_compiling():
    analyzed = analyze(
        'int main() { var parts = "a,b".split(","); return 0; }')
    assert not analyzed.errors, analyzed.errors


def test_unknown_to_unknown_stays_permissive_for_c_interop():
    # Two extern/unknown types: the analyzer cannot prove incompatibility.
