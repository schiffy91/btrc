...
RUNTIME_HELPER_ROWS: tuple[GeneratedRuntimeHelperRow, ...] = (
    ...
    GeneratedRuntimeHelperRow(
        ...
        c_source=(
            ...
            ' i = __btrc_cleanup_top; i >= 0; i--) {\n        __btrc_cleanup_entry* ex'
            'isting = &__btrc_cleanup_stack[i];\n        if (existing->try_level == __'
            'btrc_try_top && existing->slot == slot) {\n            existing->take = t'
            'ake;\n            existing->fn = fn;\n            existing->visit = visit;'
            '\n            existing->direct = direct;\n            return;\n        }\n  '
            '  }\n    if (__btrc_cleanup_cap < 1) __btrc_cleanup_cap = 64;\n    if (!__'
            'btrc_cleanup_stack) {\n        if ((size_t)__btrc_cleanup_cap > SIZE_MAX '
            '/ sizeof(__btrc_cleanup_entry)) { fprintf(stderr, "btrc: cleanup stack s'
            'ize overflow\\n"); exit(1); }\n        __btrc_cleanup_stack = (__btrc_clea'
            'nup_entry*)__btrc_safe_realloc(\n            NULL, sizeof(__btrc_cleanup_'
            'entry) * (size_t)__btrc_cleanup_cap);\n    }\n    if (__btrc_cleanup_top ='
            '= INT_MAX) { fprintf(stderr, "btrc: cleanup stack overflow\\n"); exit(1);'
            ' }\n    if (__btrc_cleanup_top + 1 >= __btrc_cleanup_cap) {\n        if (_'
            '_btrc_cleanup_cap > INT_MAX / 2) { fprintf(stderr, "btrc: cleanup stack '
            'capacity overflow\\n"); exit(1); }\n        int new_cap = __btrc_cleanup_c'
            'ap * 2;\n        if ((size_t)new_cap > SIZE_MAX / sizeof(__btrc_cleanup_e'
            'ntry)) { fprintf(stderr, "btrc: cleanup stack size overflow\\n"); exit(1)'
            '; }\n        __btrc_cleanup_stack = (__btrc_cleanup_entry*)__btrc_safe_re'
            'alloc(\n            __btrc_cleanup_stack, sizeof(__btrc_cleanup_entry) * '
            '(size_t)new_cap);\n        __btrc_cleanup_cap = new_cap;\n    }\n    __btrc'
            '_cleanup_top++;\n    __btrc_cleanup_stack[__btrc_cleanup_top] = (__btrc_c'
