...
RUNTIME_HELPER_ROWS: tuple[GeneratedRuntimeHelperRow, ...] = (
    ...
    GeneratedRuntimeHelperRow(
        ...
        c_source=(
            ...
            'st = __btrc_cleanup_top - (recent - 1);\n    if (oldest < 0) oldest = 0;\n'
            '    for (int i = __btrc_cleanup_top; i >= oldest; i--) {\n        __btrc_'
            'cleanup_entry* existing = &__btrc_cleanup_stack[i];\n        if (existing'
            '->try_level == __btrc_try_top && existing->slot == slot) {\n            e'
            'xisting->take = take;\n            existing->fn = fn;\n            existin'
            'g->visit = visit;\n            existing->direct = direct;\n            ret'
            'urn;\n        }\n    }\n    if (__btrc_cleanup_cap < 1) __btrc_cleanup_cap '
            '= 64;\n    if (!__btrc_cleanup_stack) {\n        if ((size_t)__btrc_cleanu'
            'p_cap > SIZE_MAX / sizeof(__btrc_cleanup_entry)) { fprintf(stderr, "btrc'
            ': cleanup stack size overflow\\n"); exit(1); }\n        __btrc_cleanup_sta'
            'ck = (__btrc_cleanup_entry*)__btrc_safe_realloc(\n            NULL, sizeo'
            'f(__btrc_cleanup_entry) * (size_t)__btrc_cleanup_cap);\n    }\n    if (__b'
            'trc_cleanup_top == INT_MAX) { fprintf(stderr, "btrc: cleanup stack overf'
            'low\\n"); exit(1); }\n    if (__btrc_cleanup_top + 1 >= __btrc_cleanup_cap'
            ') {\n        if (__btrc_cleanup_cap > INT_MAX / 2) { fprintf(stderr, "btr'
            'c: cleanup stack capacity overflow\\n"); exit(1); }\n        int new_cap ='
            ' __btrc_cleanup_cap * 2;\n        if ((size_t)new_cap > SIZE_MAX / sizeof'
            '(__btrc_cleanup_entry)) { fprintf(stderr, "btrc: cleanup stack size over'
            'flow\\n"); exit(1); }\n        __btrc_cleanup_stack = (__btrc_cleanup_entr'
            'y*)__btrc_safe_realloc(\n            __btrc_cleanup_stack, sizeof(__btrc_'
            'cleanup_entry) * (size_t)new_cap);\n        __btrc_cleanup_cap = new_cap;'
            '\n    }\n    __btrc_cleanup_top++;\n    __btrc_cleanup_stack[__btrc_cleanup'
            '_top] = (__btrc_cleanup_entry){\n        slot, take, fn, visit, __btrc_tr'
            'y_top, direct};\n}'
        ),
