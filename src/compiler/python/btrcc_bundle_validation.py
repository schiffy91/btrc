"""Final semantic validation for staged ``btrcc`` bundle generations."""
...
@contextmanager
def _stable_file(path: Path, max_bytes: int) -> BinaryIO:
    ...
    try:
        ...
        with os.fdopen(descriptor, "rb") as stream:
            descriptor = -1
            yield stream
            if _identity(os.fstat(stream.fileno())) != expected or _identity(path.lstat()) != expected:
                raise ValueError(f"bundle artifact changed while being validated: {path}")
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def _manifest_records(
    ...
    fields = {"format_version", "version", "target", "executable", "data_root", "files"}
    if (
    ...
    for record in manifest["files"]:
        ...
        if total_size > _MAX_PAYLOAD_BYTES:
            raise ValueError("bundle manifest payload exceeds the validation limit")
...
def _read_manifest(bundle: Path) -> bytes:
    with _stable_file(bundle / _MANIFEST_PATH, _MAX_MANIFEST_BYTES) as stream:
...
def _capture_bundle(bundle: Path, bundle_name: str, archive_name: str) -> _BundleSnapshot:
    ...
    expected_content[_MANIFEST_PATH] = hashlib.sha256(encoded).digest(), len(encoded)
...
def _expected_archive(
    ...
    files[f"{bundle_name}/{_MANIFEST_PATH}"] = hashlib.sha256(snapshot.manifest).digest(), len(snapshot.manifest)
    directories = {f"{bundle_name}/", *(f"{bundle_name}/{path}/" for path in snapshot.directories)}
...
def _hash_artifact(path: Path) -> str:
    with _stable_file(path, _MAX_ARCHIVE_BYTES) as stream:
...
def _checksum_text(path: Path) -> str:
    with _stable_file(path, 4096) as stream:
        try:
...
def validate_bundle_generation(
    bundle: Path,
    archive: Path,
    checksum: Path,
    bundle_name: str,
    ...
    snapshot = _capture_bundle(bundle, bundle_name, archive_name)
    expected_files, expected_dirs, expected_modes = _expected_archive(snapshot, bundle_name)
    checksum_before = _checksum_text(checksum)
    digest_before = _hash_artifact(archive)
    if checksum_before != f"{digest_before}  {archive_name}\n":
        raise ValueError("bundle archive checksum does not match the staged archive")
    with _stable_file(archive, _MAX_ARCHIVE_BYTES) as stream:
