...
from src.tests.debug.adapter_test_support import (
    FakeArtifact,
    FakeSession,
)
from src.tests.debug.adapter_test_support import (
...
def test_build_failure_emits_one_launch_response():
    def fail_build(*_args, **_kwargs):
        raise builder.BuildError("compiler exploded")

    def program_builder_factory(*_args, **_kwargs):
        return types.SimpleNamespace(build=fail_build)

    request = _request("launch", {"program": "/tmp/source.btrc"})
    instance = _adapter(request, FakeSession(), program_builder_factory=program_builder_factory)
...
def test_target_failure_cleans_the_new_build_artifact():
    artifact = FakeArtifact()

    def program_builder_factory(*_args, **_kwargs):
        return types.SimpleNamespace(build=lambda _program: artifact)

    request = _request("launch", {"program": "/tmp/source.btrc"})
    instance = _adapter(
        request,
        FakeSession(failing_action="create_target"),
        program_builder_factory=program_builder_factory,
    )
...
def test_launch_parses_quoted_commands_and_resolves_program_from_cwd(tmp_path):
    artifact = FakeArtifact()
    captured = {}

    def program_builder_factory(btrcpy_command, **options):
        captured["btrcpy_command"] = btrcpy_command
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"program": "/tmp/p.btrc", "btrcpy": [""]},
            "launch: 'btrcpy' command must contain only non-empty strings",
        ),
    ],
)
...
def test_configuration_response_precedes_events_and_output_is_utf8():
    ...
    instance.events.lldb = fake_lldb
    ...
    event_thread = instance.events._event_thread
...
def test_concurrent_shutdown_has_one_terminator_and_cleanup_owner():
    ...
    instance = _adapter(_request("disconnect"), session, artifact)
    first = threading.Thread(target=instance.events.terminate)
    second = threading.Thread(target=instance.events.terminate)
...
def test_event_loop_failure_terminates_and_cleans_up(capsys):
    ...
    artifact = FakeArtifact()
    instance = _adapter(_request("configurationDone"), FakeSession(), artifact)
    instance.events.lldb = types.SimpleNamespace(SBEvent=object)
