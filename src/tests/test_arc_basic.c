...
void Node_init(Node* self, int id, int value) {
    self->__rc = 1;
    (self->id = id);
    (self->value = value);
    (node_count++);
...
Container* Container_new(void) {
    Container* self = ((Container*)malloc(sizeof(Container)));
...
int main(void) {
    ...
    assert((node_count == 0));
    assert((n == NULL));
    Container* c = Container_new();
    if ((c->item != NULL)) {
        if (((--c->item->__rc) <= 0)) {
            Node_destroy(c->item);
        }
    }
    (c->item = Node_new(2, 99));
    assert((c->item->value == 99));
    assert((node_count == 1));
    Container_destroy(c);
    c = NULL;
    assert((node_count == 0));
    Node* k = Node_new(3, 77);
    assert((node_count == 1));
    (k->__rc++);
    if ((k != NULL)) {
        if (((--k->__rc) <= 0)) {
