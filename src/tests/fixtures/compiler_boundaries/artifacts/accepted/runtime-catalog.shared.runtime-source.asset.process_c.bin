...
/* btrc-runtime-helper:begin __btrc_controlling_terminal_descriptor */
static int __btrc_controlling_terminal_descriptor(void) {
    int candidates[3];
    candidates[0] = STDIN_FILENO;
    candidates[1] = STDOUT_FILENO;
    candidates[2] = STDERR_FILENO;
    for (size_t i = (size_t)0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
        if (tcgetpgrp(candidates[i]) >= (pid_t)0)
            return candidates[i];
    }
    errno = ENOTTY;
    return -1;
}
/* btrc-runtime-helper:end __btrc_controlling_terminal_descriptor */
/* btrc-runtime-helper:begin __btrc_terminal_foreground_group */
static pid_t __btrc_terminal_foreground_group(int descriptor) {
    if (descriptor < 0) { errno = EBADF; return (pid_t)-1; }
    return tcgetpgrp(descriptor);
}
/* btrc-runtime-helper:end __btrc_terminal_foreground_group */
/* btrc-runtime-helper:begin __btrc_terminal_adopt_foreground */
/* tcsetpgrp from a background process group raises SIGTTOU at the caller, so
 * the disposition is suppressed across the handoff and restored afterwards.
 * Only async-signal-safe calls are used: the child branch runs this after fork
 * and before execve. */
static int __btrc_terminal_adopt_foreground(int descriptor, pid_t group) {
    struct sigaction ignored;
    struct sigaction previous;
    int result;
    int saved;
    if (descriptor < 0 || group <= (pid_t)0) { errno = EINVAL; return -1; }
    memset(&ignored, 0, sizeof(ignored));
    ignored.sa_handler = SIG_IGN;
    if (sigemptyset(&ignored.sa_mask) != 0) return -1;
    if (sigaction(SIGTTOU, &ignored, &previous) != 0) return -1;
    result = tcsetpgrp(descriptor, group);
    saved = errno;
    if (sigaction(SIGTTOU, &previous, NULL) != 0) return -1;
    errno = saved;
    return result;
}
/* btrc-runtime-helper:end __btrc_terminal_adopt_foreground */
