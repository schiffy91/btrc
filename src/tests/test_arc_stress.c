...
typedef struct TaskHolder TaskHolder;
...
void TaskHolder_setTask(TaskHolder* self, Task* t);
...
void Task_init(Task* self, int id, int priority) {
    self->__rc = 1;
    (self->id = id);
...
Task* Task_new(int id, int priority) {
    Task* self = ((Task*)malloc(sizeof(Task)));
    memset(self, 0, sizeof(Task));
    Task_init(self, id, priority);
    return self;
}
...
void TaskHolder_init(TaskHolder* self, int seq) {
    self->__rc = 1;
    (self->task = NULL);
    (self->seq = seq);
    (holder_count++);
...
TaskHolder* TaskHolder_new(int seq) {
    TaskHolder* self = ((TaskHolder*)malloc(sizeof(TaskHolder)));
    memset(self, 0, sizeof(TaskHolder));
    TaskHolder_init(self, seq);
    return self;
}

void TaskHolder_destroy(TaskHolder* self) {
    (holder_count--);
    if ((self->task != NULL)) {
        if (((--self->task->__rc) <= 0)) {
            Task_destroy(self->task);
        }
    }
    free(self);
}
...
int main(void) {
    ...
    Task* t1 = Task_new(1, 10);
    Task* t2 = Task_new(2, 20);
    Task* t3 = Task_new(3, 30);
    assert((task_count == 3));
    TaskHolder* h1 = TaskHolder_new(1);
    TaskHolder* h2 = TaskHolder_new(2);
    assert((holder_count == 2));
    (t1->__rc++);
    TaskHolder_setTask(h1, t1);
    assert((h1->task->id == 1));
    (t2->__rc++);
    TaskHolder_setTask(h2, t2);
    assert((h2->task->id == 2));
    (t3->__rc++);
    TaskHolder_setTask(h1, t3);
    assert((h1->task->id == 3));
    assert((task_count == 3));
    TaskHolder_destroy(h1);
    h1 = NULL;
    assert((holder_count == 1));
    assert((task_count == 3));
    TaskHolder_destroy(h2);
    h2 = NULL;
    assert((holder_count == 0));
    assert((task_count == 3));
    Task_complete(t1);
    ...
    t3 = NULL;
    assert((task_count == 0));
    TaskHolder* h3 = TaskHolder_new(3);
    Task* f1 = makeTask(100, 5);
    assert((task_count == 1));
    (f1->__rc++);
    TaskHolder_setTask(h3, f1);
    assert((h3->task->id == 100));
    assert((h3->task->priority == 5));
    TaskHolder_destroy(h3);
    h3 = NULL;
    assert((holder_count == 0));
    assert((task_count == 1));
    Task_destroy(f1);
    ...
    assert((k == NULL));
    assert((task_count == 1));
    Task* d = Task_new(300, 1);
    assert((task_count == 2));
    Task_destroy(d);
    d = NULL;
    assert((d == NULL));
    assert((task_count == 1));
    TaskHolder* h4 = TaskHolder_new(4);
    Task* s1 = Task_new(401, 1);
    Task* s2 = Task_new(402, 2);
    Task* s3 = Task_new(403, 3);
    assert((task_count == 4));
    (s1->__rc++);
    TaskHolder_setTask(h4, s1);
    (s2->__rc++);
    TaskHolder_setTask(h4, s2);
    (s3->__rc++);
    TaskHolder_setTask(h4, s3);
    assert((h4->task->id == 403));
    assert((task_count == 4));
    TaskHolder_destroy(h4);
    h4 = NULL;
    Task_destroy(s1);
    s1 = NULL;
    Task_destroy(s2);
