...
static int token, active, failure, property_calls, uninitializations, disposals, registrations, stops, renders;
...
void unitReset(int requested) {
    assert(active == 0);
    failure = requested;
    property_calls = uninitializations = disposals = registrations = stops = renders = 0;
...
int unitRegistrations(void) { return registrations; }
...
OSStatus unitDispose(AudioComponentInstance unit) {
    ...
    active = 0;
    memset(&callback, 0, sizeof(callback));
    if (failure == 12) { return kAudioHardwareIllegalOperationError; }
    return noErr;
}
...
OSStatus unitStop(AudioUnit unit) {
    ...
    running = false;
    /* Successful Stop is the selected entry barrier. Failed Stop deliberately
       keeps delivery possible so tests exercise denied-entry silence. */
    memset(&callback, 0, sizeof(callback));
    return noErr;
}
...
void unitDeliver(int frames, int timestamps, int fail_input) {
    assert(active && frames > 0 && frames <= 65536);
    ...
    if (callback.inputProc) {
        ...
        memset(buffers.mBuffers[0].mData, 0, buffers.mBuffers[0].mDataByteSize);
        output_flags = kAudioUnitRenderAction_OutputIsSilence;
    }
