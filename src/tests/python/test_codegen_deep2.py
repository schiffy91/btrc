...
def test_generic_method_iterates_collection_of_class_instances():
    c = emit_c(
        "class It { public int v; public It() { self.v = 0; } public int g() { return self.v; } }\n"
        "class Reg<T> {\n"
...
def test_generic_method_calls_through_field():
    c = emit_c(
        "class Inner { public int v; public Inner() { self.v = 0; } public int get() { return self.v; } }\n"
        "class Outer<T> {\n"
...
def test_gpu_kernel_unary_and_function_call():
    c = emit_c(
        ...
        "int main() { float[] xs = {1.0}; f(xs); return 0; }"
    )
    assert "@compute" in c
...
def test_gpu_kernel_integer_local_and_expr_init_loop():
    c = emit_c(
        ...
        "    xs[i] = acc;\n"
        "}\n"
        "int main() { int[] xs = {0}; g(xs, 2); return 0; }"
    )
    assert "@compute" in c
