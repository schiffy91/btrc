...
static int catch_joined_body_error(void) {
    ...
    if (setjmp(__btrc_tls.try_stack[level]->env) == 0) {
        (void)__btrc_thread_join(worker);
        __btrc_tls.try_top--;
        return 10;
    }
    return strcmp(__btrc_tls.error_msg, "archive worker body failure") == 0
        ? 0 : 11;
}
...
static int catch_freed_arg_error(void) {
    ...
    return strcmp(__btrc_tls.error_msg, "archive worker arg failure") == 0
        ? 0 : 21;
}
