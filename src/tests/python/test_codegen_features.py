...
def test_gpu_array_kernel_emits_wgsl():
    c = emit_c(
        ...
        "int main() { int[] a = {1}; int[] b = {2}; int[] out = addv(a, b); return out[0]; }"
    )
...
def test_gpu_void_kernel_in_place_buffer():
    c = emit_c(
        ...
        "int main() { float[] xs = {1.0}; scale(xs, 2.0); return 0; }"
    )
    assert "@compute" in c
...
def test_gpu_kernel_with_control_flow():
    c = emit_c(
        ...
        "}\n"
        "int main() { int[] xs = {-1}; clamp(xs); return 0; }"
    )
    assert "@compute" in c
...
def test_gpu_multiple_scalar_uniforms():
    c = emit_c(
        ...
        "}\n"
        "int main() { float[] xs = {1.0}; affine(xs, 2.0, 1.0); return 0; }"
    )
    assert "@compute" in c
...
def test_generic_two_type_params():
    c = emit_c(
        "class Pair<K, V> { public K k; public V v;\n"
...
def test_generic_method_with_collection():
    c = emit_c(
        "class Stack<T> { public List<T> items;\n"
...
def test_thread_spawn_with_capture():
    ...
    assert "__btrc_safe_realloc(NULL, sizeof(__btrc_spawn_env" in c
...
def test_arc_field_assignment_uses_atomic_edge_replacement():
    c = emit_c(
        "class Node { public Node next; public int v; public Node() { self.v = 0; } }\n"
        "int main() { Node a = new Node(); Node b = new Node(); a.next = b; return 0; }"
    )
    assert "__btrc_arc_header __arc" in c
    assert "__btrc_arc_replace_edge" in c
...
def test_rich_enum_adt():
    c = emit_c(
        "enum class Shape { Circle(double r), Square(double s) }\n"
        "int main() { Shape shape = Shape.Circle(1.0); return shape.tag == Shape.Circle ? 0 : 1; }"
    )
    assert "Shape" in c
