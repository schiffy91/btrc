...
void Manager_setResource(Manager* self, Resource* r);
...
void Resource_init(Resource* self, int id) {
    self->__rc = 1;
    (self->id = id);
    (alive++);
}

Resource* Resource_new(int id) {
    Resource* self = ((Resource*)malloc(sizeof(Resource)));
    memset(self, 0, sizeof(Resource));
    Resource_init(self, id);
    return self;
}
...
void Manager_init(Manager* self) {
    self->__rc = 1;
    (self->res = NULL);
}
...
void Manager_destroy(Manager* self) {
    if ((self->res != NULL)) {
        if (((--self->res->__rc) <= 0)) {
            Resource_destroy(self->res);
        }
    }
    free(self);
}
...
Resource* createResource(int id) {
    Resource* r = Resource_new(id);
    return r;
}

int main(void) {
    Manager* m = Manager_new();
    Resource* r = Resource_new(1);
    assert((alive == 1));
    (r->__rc++);
    Manager_setResource(m, r);
    assert((alive == 1));
    Manager_destroy(m);
    m = NULL;
    if ((r != NULL)) {
    ...
    assert((alive == 1));
    Resource* d = Resource_new(2);
    assert((alive == 2));
    Resource_destroy(d);
    d = NULL;
    assert((d == NULL));
    assert((alive == 1));
    Resource* k = Resource_new(3);
    assert((alive == 2));
    (k->__rc++);
    if ((k != NULL)) {
        if (((--k->__rc) <= 0)) {
