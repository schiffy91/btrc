...
from src.tests.lsp.lsphelp import SAMPLE, pos_of
...
def _seed(monkeypatch, source=SAMPLE):
    ...
    monkeypatch.setattr(srv.protocol, "_workspace", _Workspace(source), raising=False)
...
def test_completion_handler(monkeypatch):
    _seed(monkeypatch)
    items = srv.complete(lsp.CompletionParams(text_document=_ident(), position=pos_of(SAMPLE, "p.getX", offset=2)))
...
def test_read_handlers_return_empty_without_cached_document(monkeypatch):
    ...
    assert srv.prepare_rename(lsp.PrepareRenameParams(text_document=_ident(), position=_ORIGIN)) is None
