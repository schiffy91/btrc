...
from src.devex.lsp.workspace import cache as unit_cache
from src.devex.lsp.workspace import units as units_mod
from src.devex.lsp.workspace.units import FileUnit
from src.devex.lsp.workspace.workspace import Workspace
from src.tests.lsp.lsphelp import SAMPLE, compute_diagnostics, get_completions, get_signature_help, pos_of, uri_to_path
...
def _install(monkeypatch, source, uri=URI):
    ...
    monkeypatch.setattr(srv.protocol, "_workspace", _Workspace(source, uri), raising=False)
...
def test_unit_cache_version_is_content_derived():
    ...
    assert v != units_mod.FrontendFingerprint().digest()  # LSP sources included
...
def test_overlay_provider_serves_open_buffer(monkeypatch):
    ...
    assert srv_state._overlay_source(target) == "class Overlay {}\n"
    assert srv_state._overlay_source("/no/such/file.btrc") is None
...
_PREPARE = REPO_ROOT / "src" / "devex" / "vscode" / "packaging" / "bundle.py"


def _load_prepare():
    spec = importlib.util.spec_from_file_location("vscode_bundle", _PREPARE)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_extension_bundler_excludes_local_state_and_artifacts(tmp_path):
    repo = tmp_path / "repo"
    ...
    (lsp_dir / "__main__.py").write_text("# server\n")
    (lsp_dir / "old.vsix").write_bytes(b"z")

    bundler = _load_prepare().ExtensionBundler(repo)
    bundle = tmp_path / "bundle"
    bundler._copy_tree(
        repo / "src" / "compiler" / "python",
        bundle / "src" / "compiler" / "python",
    )
    bundler._copy_tree(repo / "src" / "stdlib", bundle / "src" / "stdlib")
    bundler._copy_tree(repo / "src" / "language", bundle / "src" / "language")
    bundler._copy_tree(lsp_dir, bundle / "src" / "devex" / "lsp")
